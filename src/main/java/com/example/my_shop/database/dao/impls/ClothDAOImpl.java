...
public class ClothDAOImpl implements ClothDAO {
    ...
    public void create(Cloth cloth) throws SQLException {
        ...
        String ADD_CLOTH = "INSERT INTO \"Clothes\" (vendor_code, price, company_id, image, gender_id) VALUES (?,?,?,?,?)";
    ...
    public void update(Cloth cloth) throws SQLException {
        connectionPool = ConnectionPool.getInstance();
        connection = connectionPool.takeConnection();
        String UPDATE_CLOTH = "UPDATE \"Clothes\" SET vendor_code = ?, price = ?, company_id = ?, image = ?, gender_id = ? WHERE id=?";
    ...
    public Cloth getCloth(Long id) throws SQLException {
        connectionPool = ConnectionPool.getInstance();
        connection = connectionPool.takeConnection();
        String GET_CLOTH = "SELECT vendor_code, price, company_id, image, gender_id FROM \"Clothes\" WHERE id = ?";
        PreparedStatement preparedStatement = connection.prepareStatement(GET_CLOTH);
    ...
    public List<Cloth> allClothes() throws SQLException {
        ...
        connectionPool = ConnectionPool.getInstance();
        connection = connectionPool.takeConnection();
        String GET_CLOTHES = "SELECT id,vendor_code, company_id, price, image, gender_id FROM \"Clothes\" ORDER BY id";
    ...
    public List<Cloth> availableClothes() throws SQLException {
        ...
        connectionPool = ConnectionPool.getInstance();
        connection = connectionPool.takeConnection();
        String GET_AVAILABLE_CLOTHES =  "SELECT DISTINCT id, vendor_code, image, price, company_id, gender_id FROM \"Clothes\" C JOIN \"Cloth_Size\" CS on C.id = CS.cloth_id WHERE amount > 0 ";
    ...
    public void addClothAmount(Long clothId, Long sizeId, int amount) throws SQLException {
        connectionPool = ConnectionPool.getInstance();
        connection = connectionPool.takeConnection();
        String ADD_CLOTH_SIZE = "INSERT INTO \"Cloth_Size\" (cloth_id, size_id, amount) VALUES (?,?,?)";
    ...
    public void setAmountToZero(Long id) throws SQLException {
        connectionPool = ConnectionPool.getInstance();
        connection = connectionPool.takeConnection();
        String UPDATE_CLOTH = "UPDATE \"Cloth_Size\" SET amount = 0 WHERE cloth_id=?";
        try (PreparedStatement preparedStatement = connection.prepareStatement(UPDATE_CLOTH)) {
            preparedStatement.setLong(1, id);
            preparedStatement.executeUpdate();
        } finally {
